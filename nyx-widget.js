(() => {
  ...
  root.innerHTML = `
    <style>
      ...
      .nx-activity{position:relative;isolation:isolate;overflow:visible;width:max-content;padding:8px 14px;border:0;border-radius:999px;background:linear-gradient(180deg,#ffffff0d,#05080d38),rgba(10,13,18,.7);box-shadow:inset 0 0 0 1px #ffffff2b,inset 0 1px #ffffff1f,0 5px 16px #0005;color:#f4f5f7;font-size:13px;line-height:1.42;white-space:nowrap;backdrop-filter:blur(16px) saturate(145%);-webkit-backdrop-filter:blur(16px) saturate(145%);transition:width .14s ease-in-out}
  ...
  const closeImpactDetail = immediate => {
    ...
    if (immediate) { setImpactDetailPhase("closed");return }
