...
test("public widget copy uses generic infrastructure language", () => {
  assert.match(source, /Nyx runs on local Arjia hardware/)
  assert.match(source, /protected local storage for 30 days/)
  assert.match(source, /CPU-package estimate from local hardware/)
})
